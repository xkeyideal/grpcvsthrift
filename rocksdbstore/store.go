...
func (db *RocksdbStore) LookupAppliedIndex(key []byte) (uint64, error) {
	db.mu.RLock()
	defer db.mu.RUnlock()
...
func (db *RocksdbStore) Lookup(key []byte) ([]byte, error) {
	db.mu.RLock()
	defer db.mu.RUnlock()
...
func (db *RocksdbStore) NALookup(key []byte) ([]byte, error) {
	db.mu.RLock()
	defer db.mu.RUnlock()
...
func (db *RocksdbStore) BatchWrite(wb *gorocksdb.WriteBatch) error {
	return db.db.Write(db.wo, wb)
}

func (db *RocksdbStore) Write(key, value []byte) error {
	return db.db.Put(db.wo, key, value)
}

func (db *RocksdbStore) NewSnapshot() *gorocksdb.Snapshot {
	db.mu.RLock()
	defer db.mu.RUnlock()
...
func (db *RocksdbStore) ReleaseSnapshot(snap *gorocksdb.Snapshot) {
	db.db.ReleaseSnapshot(snap)
}

func (db *RocksdbStore) NewIterator(ss *gorocksdb.Snapshot) *gorocksdb.Iterator {
	ro := gorocksdb.NewDefaultReadOptions()
